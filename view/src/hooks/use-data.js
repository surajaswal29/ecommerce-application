...
export const useData = () => {
  ...
  // Order actions
  const getOrders = useCallback(() => dispatch(fetchOrders()), [dispatch])
  ...
  const clearCartData = useCallback(() => dispatch(clearCart()), [dispatch])
  ...
  // Computed values
  const cartItemsCount = data.cart.items.reduce(
    (total, item) => total + item.quantity,
    0
  )
  const cartTotal = data.cart.items.reduce(
