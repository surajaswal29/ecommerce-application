...
router
  .route('/verify_email')
  .post(UserController.verifyEmail as RequestHandler);

router
  .route('/resend_verification_otp')
  .post(UserController.resendOTP as RequestHandler);

router
  .route('/upload_file')
  .post(
    UploadImage.uploadSingleFile,
    UserController.uploadImage as RequestHandler
  );
router.route('/login').post(UserController.loginUser as RequestHandler);
router
  .route('/forgot_password')
  .post(UserController.forgotPassword as RequestHandler);
router
  .route('/password_reset/:token')
  .post(UserController.resetPassword as RequestHandler);
router.route('/logout').get(UserController.logoutUser as RequestHandler);
router
  .route('/user_info')
  .get(
    UserAuth.isAuthenticatedUser,
    UserController.getUserDetails as RequestHandler
  );
...
router
  ...
  .post(
    UserAuth.isAuthenticatedUser,
    UserController.updateUserAddress as RequestHandler
  );
router
  .route('/delete_user_address/:addressId')
  .delete(
    UserAuth.isAuthenticatedUser,
    UserController.deleteUserAddress as RequestHandler
  );
