...
export const verifyEmail = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (typeof otp !== 'number') {
    return next(new ErrorHandler('Invalid OTP: OTP must be of Number type', 400));
  }
  if (!email || !otp) {
    return next(new ErrorHandler('Please Enter All Fields', 400));
  }
...
export const resendOTP = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (user.emailVerificationExpire && user.emailVerificationExpire > new Date()) {
    return next(new ErrorHandler('OTP already sent. please wait for 2 Minutes', 400));
  }
...
export const loginUser = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const options = {
    ...
    httpOnly: true,
    sameSite: process.env.NODE_ENV === 'development' ? true : ('none' as const),
...
// Get User Details
export const getUserDetails = AsyncHandler(async (req: Types.IAuthRequest, res: Response, next: NextFunction) => {
...
// Change Password
export const updatePassword = AsyncHandler(async (req: Types.IAuthRequest, res: Response, next: NextFunction) => {
...
// Update Profile
export const updateProfile = AsyncHandler(async (req: Types.IAuthRequest, res: Response, next: NextFunction) => {
  ...
  if (!isValidKey.isValid) {
    return next(new ErrorHandler(`Invalid key: ${isValidKey.key}. Valid are: ${updateAllowed.join(', ')}`, 400));
  }

  if (!dayjs(req.body.dob, 'YYYY-MM-DD', true).isValid()) {
    return next(new ErrorHandler('Invalid date of birth format. Allowed format is YYYY-MM-DD', 400));
  }
...
export const getSingleUser = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!user) {
    return next(new ErrorHandler(`User does not exist with ${req.params.id} ID.`, 401));
  }
