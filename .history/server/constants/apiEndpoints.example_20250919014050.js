...
const productUrl = getFullUrl(API_ENDPOINTS.PRODUCT.GET_PRODUCT_DETAILS.path, 'https://api.myapp.com');
...
console.log('Does get profile require auth?', requiresAuth('USER', 'GET_MY_DETAILS'));

// Example 5: Role requirements
console.log('\n=== Role Requirements ===');
console.log('Admin user management roles:', getRequiredRoles('USER', 'GET_ALL_USERS'));
console.log('Product creation roles:', getRequiredRoles('PRODUCT', 'CREATE_PRODUCT'));
...
const validateEndpoint = (category, key, method, path) => {
  ...
  if (endpoint.method !== method) {
    return { valid: false, error: `Expected ${endpoint.method}, got ${method}` };
  }
