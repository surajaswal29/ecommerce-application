...
exports.createPayment = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    const paymentData = {
      ...
      webhook_url: webhook_url || `${process.env.BACKEND_URL}/api/v1/payment/webhook`,
...
exports.paymentWebhook = catchAsyncError(async (req, res, next) => {
  try {
    ...
    console.log('Instamojo Webhook Received:', JSON.stringify(webhookData, null, 2));
    ...
    if (payment_status === 'Credit') {
      // Payment successful
      console.log(`Payment successful for order: ${webhookData.metadata?.order_id || payment_request_id}`);
      ...
      // Payment failed
      console.log(`Payment failed for order: ${webhookData.metadata?.order_id || payment_request_id}`);
      ...
      console.log(`Payment status: ${payment_status} for order: ${webhookData.metadata?.order_id || payment_request_id}`);
    }
