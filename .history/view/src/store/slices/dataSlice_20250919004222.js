...
export const fetchProducts = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      const { keyword, page = 1, category, price, rating } = params;
      let url = `/api/v1/products?page=${page}`;
      
      if (keyword) url += `&keyword=${keyword}`;
      if (category) url += `&category=${category}`;
      if (price) url += `&price[gte]=${price[0]}&price[lte]=${price[1]}`;
      if (rating) url += `&ratings[gte]=${rating}`;
...
export const updateCartQuantity = createAsyncThunk(
  ...
  async ({ productId, quantity }, { rejectWithValue, getState }) => {
    try {
      const state = getState();
      const newItems = state.data.cart.items.map(item =>
        item.product === productId
          ? { ...item, quantity }
          : item
