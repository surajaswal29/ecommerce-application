...
export const useSnackbar = () => {
  const { showSuccess, showError, showWarning, showInfo, hideSnackbar } = useSnackbarContext();
  ...
  return {
    ...
    handleApiSuccess: (response, customMessage) => {
      const message = customMessage || response?.data?.message || 'Operation completed successfully';
      showSuccess(message);
    },
