...
const Header = () => {
  ...
  // Sync localStorage with Redux cart on component mount
  useEffect(() => {
    const savedCartItems = localStorage.getItem("cartItems");
    if (savedCartItems && JSON.parse(savedCartItems).length !== cartItems.length) {
      // If localStorage has different items than Redux, update Redux
      // This handles cases where localStorage was updated outside of Redux
      const parsedItems = JSON.parse(savedCartItems);
      if (parsedItems.length > 0) {
        // Dispatch action to sync with Redux (this would need a new action)
        // For now, we'll rely on the store initialization
      }
    }
  }, []);
