      ...
      <div className='order-card-wrapper mb-5'>
          ...
          <div className='row'>
            ...
            <div className='col-md-6 order-top-marg'>
              <p className='od-text text-start text-md-end'>
                ORDER ID: <strong>{item._id}</strong>
              </p>
              <p className='od-text text-start text-md-end'>
        ...
        <div className='item-wrapper-order'>
          {item.orderItems.map((productItems) => (
            <div
              className='item-wrapper-order item-wrapper-inner d-flex'
              key={productItems._id}
            >
              <div className='item-img'>
                <img
                  src={productItems?.images?.[0]?.url || '/placeholder-image.jpg'}
                  alt={productItems.name || 'product'}
                  onError={(e) => {
              ...
              <div className='item-name'>
                ...
                <span>
                  Product ID{' '}
                  <Link to={`/product/${productItems.product}`}>
