...
const productUrl = getCompleteUrl('PRODUCT', 'GET_PRODUCT_DETAILS', {
  id: '123',
});
console.log('Product URL with ID:', productUrl);

const productsWithFilters = getCompleteUrl(
  'PRODUCT',
  'GET_ALL_PRODUCTS',
  {},
  {
...
const paymentExamples = {
  ...
  async getPaymentStatus(paymentRequestId) {
    try {
      const result = await apiService.payment.getPaymentStatus(
        paymentRequestId
      );
